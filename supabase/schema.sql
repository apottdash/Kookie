-- =============================================================
-- WedVow Wedding Marketplace — PostgreSQL Schema
...
COMMENT ON COLUMN vendors.verified IS 'WedVow team has verified the vendor''s identity, licences, and portfolio samples.';
...
CREATE TABLE IF NOT EXISTS bookings (
    ...
    commission_amount   int,        -- computed INR commission due to WedVow
...
COMMENT ON COLUMN bookings.commission_percent IS 'WedVow platform commission rate agreed at time of booking (e.g. 10.00 = 10%).';
...
COMMENT ON COLUMN bookings.bypassed_platform IS 'True when a vendor or couple is detected to have finalised the booking outside WedVow. Used for trust-and-safety reporting.';
...
COMMENT ON VIEW vendor_inquiry_view IS 'Safe vendor-facing view of inquiries. couple_id is NULL until the couple sets couple_contact_released = true, implementing WedVow privacy-first contact model.';
